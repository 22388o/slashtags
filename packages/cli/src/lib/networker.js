import { NanoresourcePromise as Nanoresource } from 'nanoresource-promise/emitter';
...
export default class CorestoreNetworker extends Nanoresource {
  ...
  async _configure(discoveryKey, opts) {
    ...
    if (joining) {
      if (
        opts.rejoin === false &&
        prev &&
        prev.lookup === config.lookup &&
        prev.announce === config.announce
      )
        return;
...
class SwarmExtension {
  ...
  send(message, peer) {
    ...
    if (!peerExt)
      throw new Error(
        'Extension not registered for peer ' +
